...
from model.unit.ground import *
from model.unit.space import *
from model.building import *
from model.building.ground import *
from model.building.space import *
from model.world import *
from model.utils import Notification, Target, Flag
...
class View():
    ...
    def __init__(self, parent, game):
        ...
        self.root.protocol('WM_DELETE_WINDOW', self.parent.send_kill_player)
    ...
    def fGame(self):
        ...
        self.showMinerals = Label(self.ressourcesFrame, text="Minéraux: "+str(self.game.get_my_player().ressources[0]), width=20, bg="black", fg="white", anchor=NW)
        self.showMinerals.grid(column=0, row=0)
        self.showGaz = Label(self.ressourcesFrame, text="Gaz: "+str(self.game.get_my_player().ressources[1]), width=15, bg="black", fg="white", anchor=NW)
        self.showGaz.grid(column=1, row=0)
        self.showNuclear = Label(self.ressourcesFrame, text="Nucléaire: "+str(self.game.get_my_player().ressources[3]), width=15, bg="black", fg="white", anchor=NW)
        self.showNuclear.grid(column=2, row=0)
        self.showFood = Label(self.ressourcesFrame, text="Population: "+str(self.game.get_my_player().ressources[2])+"/"+str(self.game.get_my_player().MAX_FOOD), width=15, bg="black", fg="white", anchor=NW)
        ...
        self.menuModes.tradeChoice = OptionMenu(gameFrame, self.menuModes.variableTrade, *self.menuModes.tradeOPTIONS, command=self.game.ask_trade)
        self.answerId=0
        self.answerId2=0
        self.menuModes.stop_trade = Button(gameFrame, text="Arrêter l'échange", command=self.game.stop_trade)
        self.menuModes.yesButton = Button(gameFrame, text="Oui", command=lambda:self.game.start_trade(True, self.answerId))
        self.menuModes.noButton = Button(gameFrame, text="Non", command=lambda:self.game.start_trade(False, self.answerId))
        self.menuModes.yesButtonConfirm = Button(gameFrame, text="Oui", command=lambda:self.game.confirm_trade(True, self.answerId, min1, min2, gaz1, gaz2))
        self.menuModes.noButtonConfirm = Button(gameFrame, text="Non", command=lambda:self.game.confirm_trade(False, self.answerId, min1, min2, gaz1, gaz2))
        self.menuModes.modifyButtonConfirm = Button(gameFrame, text="Contre-offre", command=lambda:self.game.start_trade(True,self.game.player_id))
        ...
        self.menuModes.bEchange = Button(gameFrame,text="Échange",command=lambda:self.game.confirm_trade_question(self.answerId2))
    ...
    def fillListsAllies(self):
        for i in range(len(self.game.players)):
            if i != self.game.player_id:
                if self.game.is_allied(self.game.player_id, i):
                    if self.game.is_allied(i, self.game.player_id):
    ...
    def changeToAlly(self):
        ...
        if len(selected) > 0:
            ...
            player_id = self.game.get_player_id(playerName)
            if player_id != -1:
                if self.game.is_allied(player_id, self.game.player_id):
                ...
                self.parent.change_alliance(player_id, "Ally")
    ...
    def changeToEnnemy(self):
        ...
        if len(selected) > 0:
            ...
            if player_id != -1:
                ...
                self.parent.change_alliance(player_id, "Ennemy")
    ...
    def ongletTradeChoicePlayer(self):
        ...
        self.menuModes.variableTrade.trace('w',self.game.ask_trade)
    ...
    def ongletTradeWaiting(self):
        ...
        self.menuModes.stop_trade.grid(row=5,column=5)
    ...
    def ongletTradeYesNoQuestion(self, id1):
        ...
        self.menuModes.stop_trade.grid(row=6,column=5)
    ...
    def ongletTradeAskConfirm(self, id1, min1, min2, gaz1, gaz2):
        ...
        self.menuModes.yesButtonConfirm.config(command=lambda:self.game.confirm_trade(True, self.answerId, min1, min2, gaz1, gaz2))
        self.menuModes.noButtonConfirm.config(command=lambda:self.game.confirm_trade(False, self.answerId, min1, min2, gaz1, gaz2))
        ...
        self.menuModes.stop_trade.grid(row=7,column=5)
    ...
    def ongletTrade(self, id1, id2):
        ...
        if self.game.isMasterTrade==True:
            ...
            self.menuModes.stop_trade.grid(row=2,column=5)
            # minerals Joueurs 2
    ...
    def showInfo(self, unit):
        if isinstance(unit, Unit) or isinstance(unit, Building):
            if not isinstance(unit, ConstructionBuilding):
                ...
                if isinstance(unit, Unit):
                    ...
                    if isinstance(unit, SpaceAttackUnit) or isinstance(unit, GroundAttackUnit) or isinstance(unit, SpaceBuildingAttack) or isinstance(unit, NyanCat):
                        if isinstance(unit, SpaceBuildingAttack):
                            self.menuModes.create_image(20, 50, image = self.gifBattlecruiser)
                        elif isinstance(unit, NyanCat):
                            self.menuModes.create_image(20, 50, image = self.nyan)
                        elif isinstance(unit, SpaceAttackUnit):
                    ...
                    elif isinstance(unit, GatherShip) or isinstance(unit, GroundGatherUnit):
                        if isinstance(unit, GatherShip):
                            self.menuModes.create_image(20,50, image = self.gifCargo)
                        elif isinstance(unit, SpecialGather):
                    ...
                    elif isinstance(unit, TransportShip):
                    ...
                    elif isinstance(unit, HealingUnit):
                ...
                if len(unit.unitBeingConstruct) > 0:
                    ...
                    elif isinstance(unit, LandingZone):
    ...
    def refreshGame(self, isOnPlanet):
        self.showMinerals.config(text="Minéraux: "+str(self.game.get_my_player().ressources[0]))
        self.showGaz.config(text="Gaz: "+str(self.game.get_my_player().ressources[1]))
        self.showFood.config(text="Population: "+str(self.game.get_my_player().ressources[2])+"/"+str(self.game.get_my_player().MAX_FOOD))
        self.showNuclear.config(text="Nucléaire: "+str(self.game.get_my_player().ressources[3]))
        if self.selectedOnglet == self.SELECTED_UNIT_SELECTED:
            self.ongletSelectedUnit()
        if not isOnPlanet:
            self.drawWorld()
        else:
            self.drawPlanetGround(self.game.get_current_planet())
            self.redrawMinimap()
        y=self.HEIGHT-20
        for k in self.game.get_my_player().notifications:
    ...
    def ongletChat(self,gameFrame):
        ...
        self.parent.refresh_messages(self.menuModes.chat)
    ...
    def menuModesOnlets(self):
        ...
        self.menuModes.stop_trade.grid_forget()
    ...
    def createActionMenu(self, type):
        ...
        if(type == self.MAIN_MENU):
            ...
            units = self.game.get_my_player().selectedObjects
            if len(units) > 0:
                if isinstance(units[0], MotherShip):
                    if units[0].finished:
                        ...
                        if self.game.get_my_player().BONUS[self.game.get_my_player().ABILITY_WORM_HOLE] == 0:
                        ...
                        self.Actionmenu.create_text(15,150,text=self.drawFirstLine, anchor=NW, fill="white", font="Arial 7")
                        self.Actionmenu.create_text(15,165,text=self.drawSecondLine, anchor=NW, fill="white", font="Arial 7")
                elif isinstance(units[0], Barrack):
                    if units[0].finished:
                        ...
                        self.Actionmenu.create_text(15,150,text=self.drawFirstLine, anchor=NW, fill="white", font="Arial 7")
                        self.Actionmenu.create_text(15,165,text=self.drawSecondLine, anchor=NW, fill="white", font="Arial 7")
                elif isinstance(units[0], Utility):
                    if units[0].finished:
                        ...
                        self.Actionmenu.create_text(15,150,text=self.drawFirstLine, anchor=NW, fill="white", font="Arial 7")
                        self.Actionmenu.create_text(15,165,text=self.drawSecondLine, anchor=NW, fill="white", font="Arial 7")
                elif isinstance(units[0], Waypoint):
                    if units[0].finished:
                ...
                if len(self.game.get_my_player().selectedObjects) > 1:
                    if self.game.get_my_player().formation == self.game.get_my_player().SQUARE_FORMATION:
            ...
            self.drawFirstLine = ""
            self.drawSecondLine = ""
            way = self.game.select_waypoint_wall(self.game.get_my_player().camera.calc_point_in_world(self.positionMouse[0], self.positionMouse[1]))
            if way == None:
                ...
                self.Actionmenu.create_text(5,5,text = str(int(self.game.calc_cost_wall(self.game.get_first_unit_selected(), way)))+" gaz serait nécessaire afin de créer une muraille entre les deux.",anchor = NW, fill = 'white', width = 200)
            self.Actionmenu.create_image(140,143,image = self.gifReturn, anchor = NW, tags = 'Button_Return')
        elif(type == self.WAITING_FOR_GATHER_POINT_MENU):
            self.drawFirstLine = ""
            self.drawSecondLine = ""
            self.Actionmenu.create_text(5,5,text = "Cliquez à un endroit dans l'aire de jeu pour aller ammasser des resources.",anchor = NW, fill = 'white', width = 200)
            self.Actionmenu.create_image(140,143,image = self.gifReturn, anchor = NW, tags = 'Button_Return')
            ...
            self.Actionmenu.create_image(0,0,image=self.gifCadreMenuAction,anchor = NW, tag='actionMain')
            techTree = self.game.get_my_player().techTree
            techs = techTree.get_techs(techTree.UNITS)
            y=35
            x=13
            for i in techs:
                if x > 150:
            ...
            self.Actionmenu.create_image(0,0,image=self.gifCadreMenuAction,anchor = NW, tag='actionMain')
            techTree = self.game.get_my_player().techTree
            techs = techTree.get_techs(techTree.BUILDINGS)
            y=35
            x=13
            for i in techs:
                if x > 150:
            ...
            self.Actionmenu.create_image(0,0,image=self.gifCadreMenuAction,anchor = NW, tag='actionMain')
            techTree = self.game.get_my_player().techTree
            techs = techTree.get_techs(techTree.MOTHERSHIP)
            y=35
            x=13
            for i in techs:
                if x > 150:
    ...
    def fCreateServer(self):
        ...
        self.entryCreateServer.insert(0,self.parent.player_ip)
    ...
    def startServer(self, connect):
        if len(self.entryCreateLogin.get()) >= 3 and len(self.entryCreateLogin.get()) <= 12:
            ...
            self.parent.start_server(serverAddress, connect, userName)
        else:
    ...
    def fLobby(self):
        ...
        if self.parent.server != None:
            pNum = len(self.parent.server.sockets)
            for i in range(0, pNum):
                Label(lobbyFrame, text=self.parent.server.sockets[i][1], fg="white", bg="black", width=10).grid(row=i+3,column=0)
            Label(lobbyFrame, text='Admin : '+self.parent.server.sockets[0][1], fg="white", bg="black").grid(row=13, column=0)
            if self.game.player_id == 0:
                Button(lobbyFrame, text='Demarrer la partie', command=self.parent.start_game, bg="black", fg="white").grid(row=13, column=1)
                Button(lobbyFrame, text='Ajouter un IA', command=self.parent.add_ai, bg="black", fg="white").grid(row=12, column=1)
        ...
        listOfColors = self.parent.server.choiceColors
        self.colorOPTIONS = ["Orange","Rouge","Bleu"]
        self.variableColor.set(self.colorOPTIONS[0]) # default value
        self.colorChoice = OptionMenu(lobbyFrame, self.variableColor, *self.colorOPTIONS, command=self.parent.choose_color)
        self.colorChoice.grid(row=(self.game.player_id)+3, column=1)
        self.colorChoice['menu'].delete(0, END)
        for i in listOfColors:
            if i[1] == False or self.parent.server.sockets[self.game.player_id][3] == listOfColors.index(i):
        ...
        self.variableColor.trace('w',self.parent.choose_color)
        return lobbyFrame
    ...
    def redrawLobby(self ,lobbyFrame):
        ...
        if self.parent.server != None:
            ...
            for i in range(0, pNum):
                Label(lobbyFrame, text=self.parent.server.sockets[i][1], fg="white", bg="black", width=10).grid(row=i+3,column=0)
                if self.parent.server.sockets[i][3] != -1 and i != self.game.player_id:
                    Label(lobbyFrame, text=self.parent.server.choiceColors[self.parent.server.sockets[i][3]][0], fg="white", bg="black").grid(row=i+3, column=1)
                
    def sendMessLobby(self, eve):
        if self.entryMessLobby.get() != "":
            self.parent.send_message_lobby(self.entryMessLobby.get(), self.parent.server.sockets[self.game.player_id][1])
    ...
    def drawPlanetGround(self, planet):
        ...
        for i in planet.minerals:
            if i.nbMinerals > 0:
                distance = self.game.get_my_player().camera.calc_distance(i.position)
                if i in self.game.get_my_player().selectedObjects:
        ...
        if planet.nuclearSite != None:
            if planet.nuclearSite.nbRessource > 0:
                ...
                distance = self.game.get_my_player().camera.calc_distance(site.position)
                if site in self.game.get_my_player().selectedObjects:
        ...
        for i in planet.landingZones:
            if i.is_alive:
                ...
                if i in self.game.get_my_player().selectedObjects:
                    if i.owner == self.game.player_id:
                        rallyView = self.game.get_my_player().camera.calc_distance(i.rallyPoint)
        ...
        if self.isSettingBuildingPosition:
            if self.game.get_my_player().can_afford(Building.COST[self.buildingToBuild][0],Building.COST[self.buildingToBuild][1],0):
                self.drawFuturBuilding()
            else:
                self.game.get_my_player().notifications.append(Notification([-10000,-10000,-10000], Notification.NOT_ENOUGH_RESSOURCES))
                self.isSettingBuildingPosition = False
    ...
    def drawUnitGround(self, unit, color):
        distance = self.game.get_my_player().camera.calc_distance(unit.position)
        if unit in self.game.get_my_player().selectedObjects:
        ...
        if unit.type == Unit.SPECIAL_GATHER:
            if unit.isGathering and unit in self.game.get_my_player().selectedObjects:
                ...
                self.gameArea.create_rectangle(distance[0]-(unit.SIZE[unit.type][0]/2+4), distance[1]-(unit.SIZE[unit.type][1]/2+15),distance[0]-(unit.SIZE[unit.type][0]/2+4)+unit.calc_progression(), distance[1]-(unit.SIZE[unit.type][1]/2+5), outline='black', fill=unit.get_color_progression(), tag='deletable')
            ...
            if unit.attack_count <= 5 and unit.flag.flagState == Flag.ATTACK:
                d2 = self.game.get_my_player().camera.calc_distance(unit.flag.finalTarget.position)
    ...
    def drawBuildingGround(self, building, color):
        distance = self.game.get_my_player().camera.calc_distance(building.position)
        if building in self.game.get_my_player().selectedObjects:
    ...
    def drawPlanetBackground(self):
        ...
        camera = self.game.get_my_player().camera
        pos = camera.calc_distance([0,0])
    ...
    def drawWorld(self):
        self.gameArea.delete('deletable')
        sunList = self.game.galaxy.solar_system_list
        players = self.game.players 
        id = self.game.player_id
        for i in self.game.galaxy.wormholes:
            if self.game.get_my_player().in_view_range(i.position):
                self.drawWormHole(i, players[id])
        for i in sunList:
            if self.game.get_my_player().in_view_range(i.sunPosition):
                if not i.discovered:
                    i.discovered = True
                    self.redrawMinimap()
            ...
            for j in i.planets:
                if self.game.get_my_player().in_view_range(j.position) or j.already_landed(id):
                    if not j.discovered:
                        j.discovered = True
                        self.redrawMinimap()
            ...
            for j in i.nebulas:
                if self.game.get_my_player().in_view_range(j.position):
                    if not j.discovered:
                        j.discovered = True
                        self.redrawMinimap()
        ...
        for i in players:
            if i.is_alive:
                for j in i.buildings:
                    if j.is_alive and not isinstance(j, GroundBuilding):
                        if i.id == self.game.player_id:
                            if j in i.selectedObjects and isinstance(j, ConstructionBuilding) and not isinstance(j, LandingZone) and j.finished:
                                rallyView = i.camera.calc_distance(j.rallyPoint)
                                self.gameArea.create_image(rallyView[0],rallyView[1], image = self.gifRally, tag='deletable')
                        if self.game.get_my_player().in_view_range(j.position):
                            self.drawBuilding(j,i,False)
                            if i.id != self.game.player_id:
                                self.game.add_building_enemy(j, j.finished)
                for j in i.units:
                    if j.is_alive and not isinstance(j, GroundUnit):
                        if self.game.get_my_player().in_view_range(j.position):
                            self.drawUnit(j, i, False)
                for j in i.walls:
                    if self.game.get_my_player().in_view_range(j.wp1.position) or self.game.get_my_player().in_view_range(j.wp2.position):
                        self.drawWall(j, i)
                for j in i.bullets:
                    self.drawBullet(j, i, False)
                for j in i.buildingsFound:
                    if j[0].is_alive:
                        if not self.game.get_my_player().in_view_range(j[0].position):
    ...
    def drawWormHole(self, wormHole, player):
        if wormHole.duration > 0:
            if player.camera.is_in_field_of_view(wormHole.position):
                distance = player.camera.calc_distance(wormHole.position)
                self.gameArea.create_image(distance[0], distance[1], image=self.wormhole, tag='deletable')
            if player.camera.is_in_field_of_view(wormHole.destination):
                distance = player.camera.calc_distance(wormHole.destination)
                self.gameArea.create_image(distance[0], distance[1], image=self.wormhole, tag='deletable')

    #Pour dessiner un soleil     
    def drawSun(self, sunPosition, player, isInFOW):
        if player.camera.is_in_field_of_view(sunPosition):
            distance = player.camera.calc_distance(sunPosition)
            if isInFOW:
    ...
    def drawPlanet(self, planet, player, isInFOW):
        ...
        if player.camera.is_in_field_of_view(planetPosition):
            distance = player.camera.calc_distance(planetPosition)
            if isInFOW:
                if planet in player.selectedObjects:
                    if planet.already_landed(player.id):
                    ...
                    mVariable = "Mineral :" + str(planet.get_num_minerals())
                    gVariable = "Gaz :" + str(planet.get_num_gaz())
    ...
    def drawNebula(self,nebula,player, isInFOW):
        ...
        if nebula.gazQte > 0:
            if player.camera.is_in_field_of_view(nebulaPosition):
                distance = player.camera.calc_distance(nebulaPosition)
                if isInFOW:
    ...
    def drawAsteroid(self,asteroid,player, isInFOW):
        ...
        if asteroid.mineralQte > 0:
            if player.camera.is_in_field_of_view(asteroidPosition):
                distance = player.camera.calc_distance(asteroidPosition)
                if isInFOW:
    ...
    def drawBuilding(self, building,  player, isInFOW):
        ...
        if self.game.get_my_player().camera.is_in_field_of_view(buildingPosition):
            distance = self.game.get_my_player().camera.calc_distance(buildingPosition)
            if not isInFOW:
    ...
    def drawWall(self, wall, player):
        distance1 = self.game.get_my_player().camera.calc_distance(wall.wp1.position)
        distance2 = self.game.get_my_player().camera.calc_distance(wall.wp2.position)
    ...
    def drawFuturBuilding(self):
        buildingPos = self.game.get_my_player().camera.calc_point_in_world(self.positionMouse[0], self.positionMouse[1])
        distance = self.game.get_my_player().camera.calc_distance(buildingPos)
        building = self.buildingToBuild
        planet = self.game.get_current_planet()
        if planet != None:
            if self.game.check_if_can_build(buildingPos, building, planet_id = planet.id, sun_id = planet.solarSystem.sun_id):
                color = "green"
            else:
                color = "red"
        else:
            if self.game.check_if_can_build(buildingPos, building):
                color = "green"
            else:
                color = "red"
        self.gameArea.create_rectangle(distance[0]-Building.SIZE[building][0]/2, distance[1]-Building.SIZE[building][1]/2, distance[0]+Building.SIZE[building][0]/2, distance[1]+Building.SIZE[building][1]/2, fill=color, outline="lightgray", tag='deletable')
        if building == Building.WAYPOINT:
            self.gameArea.create_image(distance[0], distance[1], image=self.waypoints[self.game.get_my_player().colorId], tag='deletable')
        elif building == Building.BARRACK:
            self.gameArea.create_image(distance[0], distance[1], image=self.barracks[self.game.get_my_player().colorId], tag='deletable')
        elif building == Building.UTILITY:
            self.gameArea.create_image(distance[0], distance[1], image=self.utilities[self.game.get_my_player().colorId], tag='deletable')
        elif building == Building.TURRET:
            self.gameArea.create_image(distance[0], distance[1], image=self.turrets[self.game.get_my_player().colorId], tag='deletable')
        elif building == Building.LAB:
            self.gameArea.create_image(distance[0], distance[1], image=self.labs[self.game.get_my_player().colorId], tag='deletable')
        elif building == Building.FARM:
            self.gameArea.create_image(distance[0], distance[1], image=self.farms[self.game.get_my_player().colorId], tag='deletable')
        elif building == Building.MOTHERSHIP:
            self.gameArea.create_image(distance[0], distance[1], image=self.motherShips[self.game.get_my_player().colorId], tag='deletable')

    def drawCircleRange(self):
        self.gameArea.create_oval(self.positionMouse[0]-100, self.positionMouse[1]-100, self.positionMouse[0]+100, self.positionMouse[1]+100, outline=self.laserColors[self.game.get_my_player().colorId], tag='deletable')

    def drawBullet(self, bullet, player, isInFOW):
        pos = self.game.get_my_player().camera.calc_distance(bullet.position)
    ...
    def drawUnit(self, unit, player, isInFOW):
        ...
        if self.game.get_my_player().camera.is_in_field_of_view(unitPosition):
            distance = self.game.get_my_player().camera.calc_distance(unitPosition)
            if not isInFOW:
    ...
    def drawHPHoverUnit(self, unit, distance):
        posSelected=self.game.get_my_player().camera.calc_point_in_world(self.positionMouse[0],self.positionMouse[1])
        if unit.position[0] >= posSelected[0]-(unit.SIZE[unit.type][0]/2) and unit.position[0] <= posSelected[0]+(unit.SIZE[unit.type][0]/2):
            if unit.position[1] >= posSelected[1]-(unit.SIZE[unit.type][1]/2) and unit.position[1] <= posSelected[1]+(unit.SIZE[unit.type][1]/2):
                if unit.owner != self.game.player_id:
    ...
    def drawHPBars(self, distance, unit):
        if unit.hitpoints/unit.MAX_HP[unit.type] <= 0.2:
            ...
            color = "green"
        player = self.game.get_my_player()
        if ((isinstance(unit, GroundBuilding) or isinstance(unit, GroundUnit) or unit.type == Building.LANDING_ZONE) and player.current_planet != None) or ((isinstance(unit, SpaceBuilding) or isinstance(unit, SpaceUnit) or unit.type == Unit.SCOUT or isinstance(unit, ConstructionBuilding) and not isinstance(unit, LandingZone) and player.current_planet == None)):
    ...
    def drawMinimap(self):
        ...
        if self.game.get_my_player().current_planet == None:
            sunList = self.game.galaxy.solar_system_list
            players = self.game.players
            if self.firstTime:
                for i in sunList:
            ...
            for i in players:
                if self.game.get_my_player().is_ally(i.id):
                    for j in i.units:
                        if j.is_alive and not isinstance(j, GroundUnit):
                                self.drawMiniUnit(j)
                    for j in i.buildings:
                        if j.is_alive and not isinstance(j, GroundBuilding) and not isinstance(j, LandingZone):
                            if j.finished:
                    ...
                    for j in i.units:
                        if j.is_alive and not isinstance(j, GroundUnit):
                            if players[self.game.player_id].in_view_range(j.position):
                                self.drawMiniUnit(j)
                    for j in i.buildings:
                        if j.is_alive and not isinstance(j, GroundBuilding) and not isinstance(j, LandingZone):
                            if j.finished:
            ...
            planet = self.game.get_my_player().current_planet
            for i in planet.minerals:
        ...
        if self.game.get_my_player().current_planet == None:
            ...
            for i in sunList:
                ...
                for q in i.asteroids:
                    self.drawMiniAsteroid(q)
            for i in players:
                if players[self.game.player_id].is_ally(i.id):
                    for j in i.units:
                        if j.is_alive and not isinstance(j, GroundUnit):
                            self.drawMiniUnit(j)
                    for j in i.buildings:
                        if j.is_alive:
                            if j.finished and not isinstance(j, GroundBuilding) and not isinstance(j, LandingZone):
                                self.drawMiniBuilding(j)
                    if i.id == self.game.player_id:
                        for j in i.buildingsFound:
                            if j[0].is_alive:
    ...
    def drawMiniNotification(self,notification, y):
        if self.game.get_current_planet() == None:
        ...
        if notification.refreshSeen >= 60:
            self.game.get_my_player().notifications.remove(notification)
    ...
    def drawMiniUnit(self, unit):
        ...
        if unit.owner == self.game.player_id:
            color = 'GREEN'
        elif self.game.get_my_player().is_ally(unit.owner):
            color = 'YELLOW'
        else:
            color ='RED'
    ...
    def drawMiniBuilding(self, building):
        ...
        if building.owner == self.game.player_id:
            color = 'GREEN'
        elif self.game.get_my_player().is_ally(building.owner):
            color = 'YELLOW'
        else:
            color ='RED'
    ...
    def drawMiniLandingZone(self, zone, planet):
        ...
        if zone.ownerId == self.game.player_id:
                color = 'WHITE'
        elif self.game.get_my_player().is_ally(zone.ownerId):
            color ='YELLOW'
        else:
            color ='RED'
    ...
    def keyPressUP(self, eve):
        if 'UP' not in self.game.get_my_player().camera.movingDirection:
            self.game.get_my_player().camera.movingDirection.append('UP')
            if self.game.get_my_player().current_planet == None:
                self.drawWorld()
            else:
                self.drawPlanetGround(self.game.get_my_player().current_planet)

    def keyPressDown(self, eve):
        if 'DOWN' not in self.game.get_my_player().camera.movingDirection:
            self.game.get_my_player().camera.movingDirection.append('DOWN')
            if self.game.get_my_player().current_planet == None:
                self.drawWorld()
            else:
                self.drawPlanetGround(self.game.get_my_player().current_planet)
    ...
    def keyPressLeft(self, eve):
        if 'LEFT' not in self.game.get_my_player().camera.movingDirection:
            self.game.get_my_player().camera.movingDirection.append('LEFT')
            if self.game.get_my_player().current_planet == None:
                self.drawWorld()
            else:
                self.drawPlanetGround(self.game.get_my_player().current_planet)
    ...
    def keyPressRight(self, eve):
        if 'RIGHT' not in self.game.get_my_player().camera.movingDirection:
            self.game.get_my_player().camera.movingDirection.append('RIGHT')
            if self.game.get_my_player().current_planet == None:
                self.drawWorld()
            else:
                self.drawPlanetGround(self.game.get_my_player().current_planet)
    ...
    def keyReleaseUP(self, eve):
        if 'UP' in self.game.get_my_player().camera.movingDirection:
            self.game.get_my_player().camera.movingDirection.remove('UP')
        if self.game.get_my_player().current_planet == None:
            self.drawWorld()
        else:
            self.drawPlanetGround(self.game.get_my_player().current_planet)
    ...
    def keyReleaseDown(self, eve):
        if 'DOWN' in self.game.get_my_player().camera.movingDirection:
            self.game.get_my_player().camera.movingDirection.remove('DOWN')
        if self.game.get_my_player().current_planet == None:
            self.drawWorld()
        else:
            self.drawPlanetGround(self.game.get_my_player().current_planet)
    ...
    def keyReleaseLeft(self, eve):
        if 'LEFT' in self.game.get_my_player().camera.movingDirection:
            self.game.get_my_player().camera.movingDirection.remove('LEFT')
        if self.game.get_my_player().current_planet == None:
            self.drawWorld()
        else:
            self.drawPlanetGround(self.game.get_my_player().current_planet)
    ...
    def keyReleaseRight(self, eve):
        if 'RIGHT' in self.game.get_my_player().camera.movingDirection:
            self.game.get_my_player().camera.movingDirection.remove('RIGHT')
        if self.game.get_my_player().current_planet == None:
            self.drawWorld()
        else:
            self.drawPlanetGround(self.game.get_my_player().current_planet)
        ...
        if x > 0 and x < self.WIDTH:
            if y > 0 and y < self.WIDTH-200:
                if canva == self.gameArea:
                    pos = self.game.get_my_player().camera.calc_point_in_world(x,y)
                    self.game.right_click(pos)
                elif canva == self.minimap:
                    pos = self.game.get_my_player().camera.calc_point_mini_map(x,y)
                    if len(self.game.get_my_player().selectedObjects) > 0:
                        if isinstance(self.game.get_my_player().selectedObjects[0], ConstructionBuilding):
                            self.game.set_rally_point_position(pos)
                    if self.game.get_current_planet() == None:
                        self.game.set_moving_flag(pos[0], pos[1])
    ...
    def leftclic(self, eve):
        ...
        if canva == self.gameArea:
            ...
            if self.attacking or self.isSettingAttackPosition:
                self.game.select_unit_enemy(pos)
                self.isSettingAttackPosition = False
                self.actionMenuType = self.MAIN_MENU
                ...
                self.game.create_worm_hole(pos)
                self.isSettingWormHole = False
                self.actionMenuType = self.MAIN_MENU
                
            elif self.isSettingWallsPosition:
                self.game.set_linked_waypoint(pos)
                self.isSettingWallsPosition = False
                self.actionMenuType = self.MAIN_MENU

            elif self.isSettingAttackBuildingPosition:
                self.game.set_attack_building_flag(pos)
                self.isSettingAttackBuildingPosition = False
                self.actionMenuType = self.MAIN_MENU
                
            elif self.isSettingBuildingPosition or self.building:
                if Building.INSPACE[self.buildingToBuild] == True:
                    self.game.set_building_flag(pos[0],pos[1], self.buildingToBuild)
                else:
                    self.game.set_building_flag(pos[0],pos[1], self.buildingToBuild, self.sun_id, self.planet_id)
                self.isSettingBuildingPosition = False
                self.actionMenuType = self.MAIN_MENU
                
            elif self.isChosingUnitToHeal :
                self.game.select_unit_to_heal(pos)
                self.isChosingUnitToHeal = False
                self.actionMenuType = self.MAIN_MENU
    ...
    def endDrag(self, eve):
        ...
        if self.dragging:
            self.dragging = False
            self.selectEnd = [eve.x, eve.y]
            self.game.box_select(self.selectStart, self.selectEnd)
            self.ongletSelectedUnit()
    ...
    #methode test attack
    def attack(self,eve):
        attackType = self.game.can_set_attack()
    ...
    def enter(self, eve):
        self.parent.send_message(self.menuModes.entryMess.get())
    ...
    def lobbyEnter(self, eve, login="", ns=""):
        ...
        if len(login) >= 3 and len(login) <= 12:
            self.parent.connect_server(login,ns)
        else:
            self.showTooDamnShortName()
			
    def stop(self, eve):
        self.attacking = False
        self.game.set_standby_flag()

    def delete(self, eve):
        self.attacking = False
        self.game.erase_unit()
    ...
    def checkMotherShip(self, eve):
        self.game.get_my_player().current_planet = None
        self.isSettingOff()
        cam = self.game.get_my_player().camera
        self.game.get_my_player().motherCurrent += 1
        if self.game.get_my_player().motherCurrent >= len(self.game.get_my_player().motherships):
            self.game.get_my_player().motherCurrent = 0
        mothership = self.game.get_my_player().motherships[self.game.get_my_player().motherCurrent]
        cam.position = [mothership.position[0], mothership.position[1]]
        self.game.get_my_player().selectedObjects = []
        self.game.get_my_player().selectedObjects.append(mothership)
        self.changeBackground('GALAXY')
        self.actionMenuType = self.MAIN_MENU
        self.drawWorld()
        self.redrawMinimap()
    ...
    def getOutPlanet(self, eve):
        if self.game.get_my_player().current_planet != None:
            ...
            planet = self.game.get_my_player().current_planet = None
            self.game.get_my_player().selectedObjects = []
            self.changeBackground('GALAXY')
            self.drawWorld()
            self.redrawMinimap()
            ...
            if self.game.get_my_player().current_planet == None:
                ...
                for i in planet.landingZones:
                    if i.ownerId == player.id:
                        landingZone = i
                player.camera.place_on_landing(landingZone)
                self.game.get_my_player().current_planet = planet
                self.game.get_my_player().selectedObjects = []
        ...
        if bp != ():
            ...
            if (Button_pressed == "bouton_chat"):
                ...
                if self.game.tradePage==-1:
                    ...
                    if self.game.isMasterTrade == True:
                        self.ongletTrade(self.game.player_id,self.game.idTradeWith)
                    else:
                        self.ongletTrade(self.game.idTradeWith,self.game.player_id)
                ...
                self.game.select_object_from_menu(int(bp[1]))
                self.ongletSelectedUnit()
            elif (Button_pressed == 'cancelUnitButton'):
                self.game.send_cancel_unit(bp[1])
            elif (Button_pressed == 'cancelTechButton'):
                self.game.send_cancel_tech(bp[1])
            elif (Button_pressed == 'selected_all_units'):
                self.game.select_unit_by_type(int(bp[1]))
    ...
    def takeOff(self, eve):
        ...
        if planet != None:
            for i in planet.landingZones:
                if i.ownerId == self.game.player_id and i.LandedShip != None:
                    if i in self.game.get_my_player().selectedObjects:
                        self.game.set_take_off_flag(i.LandedShip, planet)
                        self.actionMenuType = self.MAIN_MENU
                        self.game.get_my_player().selectedObjects = []
    ...
    def clickActionMenu(self,eve):
        ...
        if bp != ():
            ...
            if (Button_pressed == "Button_Stop"):
                ...
                if self.game.get_my_player().can_afford(Building.COST[Building.WAYPOINT][0],Building.COST[Building.WAYPOINT][1],0):
                    self.buildingToBuild = Building.WAYPOINT
                    self.isSettingBuildingPosition = True;
            elif (Button_pressed == "Button_Build_Turret"):
                self.actionMenuType = self.WAITING_FOR_BUILDING_POINT_MENU
                self.buildingToBuild = Building.TURRET
                self.isSettingBuildingPosition = True;
            elif (Button_pressed == "Button_Build_MotherShip"):
                self.actionMenuType = self.WAITING_FOR_BUILDING_POINT_MENU
                self.buildingToBuild = Building.MOTHERSHIP
                self.isSettingBuildingPosition = True;
            elif (Button_pressed == "Button_Build_A_Barrack"):
                self.actionMenuType = self.WAITING_FOR_BUILDING_POINT_MENU
                self.buildingToBuild = Building.BARRACK
                self.isSettingBuildingPosition = True;
            elif (Button_pressed == "Button_Build_A_Utility"):
                self.actionMenuType = self.WAITING_FOR_BUILDING_POINT_MENU
                self.buildingToBuild = Building.UTILITY
                self.isSettingBuildingPosition = True;
            elif (Button_pressed == "Button_Build_Farm"):
                self.actionMenuType = self.WAITING_FOR_BUILDING_POINT_MENU
                self.buildingToBuild = Building.FARM
                self.isSettingBuildingPosition = True;
                unit = self.game.get_my_player().get_first_unit()
                self.sun_id = unit.sun_id
                self.planet_id = unit.planet_id
            elif (Button_pressed == "Button_Build_Lab"):
                self.actionMenuType = self.WAITING_FOR_BUILDING_POINT_MENU
                self.buildingToBuild = Building.LAB
                self.isSettingBuildingPosition = True;
                unit = self.game.get_my_player().get_first_unit()
                self.sun_id = unit.sun_id
                self.planet_id = unit.planet_id
                ...
                self.game.add_unit(Unit.SCOUT)
            elif (Button_pressed == "Button_Build_Attack"):
                self.game.add_unit(Unit.ATTACK_SHIP)
            elif (Button_pressed == "Button_Build_Transport"):
                self.game.add_unit(Unit.TRANSPORT)
            elif (Button_pressed == "Button_Build_Gather"):
                self.game.add_unit(Unit.CARGO)
            elif (Button_pressed == "Button_Build_Special"):
                self.game.add_unit(Unit.SPECIAL_GATHER)
            elif (Button_pressed == 'Button_Build_Healer'):
                self.game.add_unit(Unit.HEALING_UNIT)
            elif (Button_pressed == 'Button_Build_GroundAttack'):
                self.game.add_unit(Unit.GROUND_ATTACK)
            elif (Button_pressed == 'Button_Build_GroundGather'):
                self.game.add_unit(Unit.GROUND_GATHER)
            elif (Button_pressed == 'Button_Build_GroundBuild'):
                self.game.add_unit(Unit.GROUND_BUILDER_UNIT)
            elif (Button_pressed == "Button_Triangle"):
                self.game.set_change_formation_flag(self.game.get_my_player().TRIANGLE_FORMATION)
            elif (Button_pressed == "Button_Square"):
                self.game.set_change_formation_flag(self.game.get_my_player().SQUARE_FORMATION)
                ...
                self.game.add_unit(Unit.SPACE_BUILDING_ATTACK)
                ...
                self.game.set_buy_tech(Button_pressed[0], Button_pressed[1])
    ...
    def detailAction(self, eve):
        ...
        if bp != ():
            ...
            if (Button_pressed == "Button_Build_Scout"):
                ...
                self.drawFirstLine=str(Building.NAME[Building.WAYPOINT])
                self.drawSecondLine=str(Building.COST[Building.WAYPOINT][0])+" mine | "+str(Building.COST[Building.WAYPOINT][1])+" gaz"
            elif (Button_pressed == "Button_Build_Turret"):
                self.drawFirstLine=str(Building.NAME[Building.TURRET])
                self.drawSecondLine=str(Building.COST[Building.TURRET][0])+" mine | "+str(Building.COST[Building.TURRET][1])+" gaz"
            elif (Button_pressed == "Button_Build_MotherShip"):
                self.drawFirstLine=str(Building.NAME[Building.MOTHERSHIP])
                self.drawSecondLine=str(Building.COST[Building.MOTHERSHIP][0])+" mine | "+str(Building.COST[Building.MOTHERSHIP][1])+" gaz"
            elif (Button_pressed == "Button_Build_Lab"):
                self.drawFirstLine=str(Building.NAME[Building.LAB])
                self.drawSecondLine=str(Building.COST[Building.LAB][0])+" mine | "+str(Building.COST[Building.LAB][1])+" gaz"
            elif (Button_pressed == "Button_Build_A_Barrack"):
                self.drawFirstLine=str(Building.NAME[Building.BARRACK])
                self.drawSecondLine=str(Building.COST[Building.BARRACK][0])+" mine | "+str(Building.COST[Building.BARRACK][1])+" gaz"
            elif (Button_pressed == "Button_Build_A_Utility"):
                self.drawFirstLine=str(Building.NAME[Building.UTILITY])
                self.drawSecondLine=str(Building.COST[Building.UTILITY][0])+" mine | "+str(Building.COST[Building.UTILITY][1])+" gaz"
            elif (Button_pressed == "Button_Build_Farm"):
                self.drawFirstLine=str(Building.NAME[Building.FARM])
                self.drawSecondLine=str(Building.COST[Building.FARM][0])+" mine | "+str(Building.COST[Building.FARM][1])+" gaz"
            elif (Button_pressed == "Button_Build_GroundAttack"):
                ...
                if Button_pressed[0] == 'Button_Buy_Building_Tech':
                    techTree = self.game.get_my_player().techTree
                    tech = techTree.get_techs(techTree.BUILDINGS)[int(Button_pressed[1])]
                    ...
                    techTree = self.game.get_my_player().techTree
                    tech = techTree.get_techs(techTree.UNITS)[int(Button_pressed[1])]
                    self.drawFirstLine=tech.name
                    self.drawSecondLine="M:"+str(tech.costMine)+" G:"+str(tech.costGaz)+" N:"+str(tech.costNuclear)
                elif Button_pressed[0] == 'Button_Buy_MotherShip_Tech':
                    techTree = self.game.get_my_player().techTree
                    tech = techTree.get_techs(techTree.MOTHERSHIP)[int(Button_pressed[1])]
                    self.drawFirstLine=tech.name
                    self.drawSecondLine="M:"+str(tech.costMine)+" G:"+str(tech.costGaz)+" N:"+str(tech.costNuclear)
            else:
                self.drawFirstLine=""
    ...
    def ping(self, eve):
        pos = self.game.get_my_player().camera.calc_point_on_map(eve.x, eve.y)
        self.game.ping_allies(pos[0], pos[1])

    def deleteAll(self):
        self.gameArea.delete(ALL)

    def selectMemory(self, eve):
        self.game.select_memory(int(eve.char)-1)
        
    def newMemory(self, eve):
        self.game.new_memory(eve.keycode-112)
