...
class SpaceAttackUnit(SpaceUnit):
    def __init__(self,  type, position, owner):
        super(SpaceAttackUnit, self).__init__(type, position, owner)
    ...
    def action(self, parent):
        if self.flag.flag_state == Flag.ATTACK:
            ...
            super(SpaceAttackUnit, self).action(parent)

    def change_flag(self, final_target, state):
        self.attack_count = self.attack_speed
        super(SpaceAttackUnit, self).change_flag(final_target, state)
    ...
    def apply_bonuses(self, bonuses):
        super(SpaceAttackUnit, self).apply_bonuses(bonuses)
